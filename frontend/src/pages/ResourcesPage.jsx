...
import { ArrowUpRight, Library, Plus, FileText, Image as ImageIcon, Video, Music, File, ExternalLink } from 'lucide-react';
import { useGetResources } from '../services/apiHooks.js';
...
function ResourceIcon({ type, mimeType }) {
  if (mimeType?.includes('image')) return <ImageIcon size={17} />;
  if (mimeType?.includes('video')) return <Video size={17} />;
  if (mimeType?.includes('audio')) return <Music size={17} />;
  if (type === 'Document' || mimeType?.includes('pdf')) return <FileText size={17} />;
  if (type === 'Presentation') return <File size={17} />;
  if (type === 'YouTube') return <ExternalLink size={17} />;
  ...
  return (
    <Shell>
      <PageHeading
        ...
        action={<Button onClick={() => setOpen(true)}><Plus size={16} /> Save resource</Button>}
      />
      ...
      {query.isLoading ? (
        ...
        <EmptyState
          ...
          action={<Button onClick={() => setOpen(true)}><Plus size={16} /> Save first resource</Button>}
        />
      ) : (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {resources.map((r) => {
            ...
            return (
              <article key={r.id} className="card-lift flex flex-col rounded-2xl border border-card-border bg-card p-5">
                <div className="flex items-start justify-between">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-secondary text-secondary-foreground">
                    <ResourceIcon type={r.resourceType} mimeType={r.fileData?.mimeType} />
                  </div>
                  <span className={cx('rounded-full px-2 py-1 font-mono text-[9px] uppercase', r.watched ? 'bg-accent/15 text-accent' : 'bg-muted text-muted-foreground')}>
                    {r.watched ? 'completed' : 'to explore'}
                  </span>
                </div>
                <h2 className="mt-5 font-display text-2xl leading-tight">{r.title}</h2>
                ...
                {isFile && r.fileData?.mimeType?.startsWith('audio/') && (
                  <div className="mt-4">
                ...
                <div className="mt-3 flex items-center gap-2 text-xs">
                  <span className="font-semibold">{r.subject?.name || r.subject || 'General'}</span>
                ...
                <div className="mt-auto flex items-center justify-between border-t border-border pt-4 mt-5">
                  ...
                  {targetUrl && (
                    <a href={targetUrl} target="_blank" rel="noreferrer" className="focus-ring inline-flex items-center gap-1 text-xs font-bold text-accent hover:underline">
                      {isFile ? 'Open File' : 'Open Link'} <ArrowUpRight size={13} />
                    </a>
                  )}
                </div>
              </article>
