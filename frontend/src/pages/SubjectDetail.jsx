...
import { BookOpen, CheckCircle, Circle, ArrowLeft, Plus, ChevronDown, ChevronRight, Play, Wand2 } from 'lucide-react';
...
export default function SubjectDetail() {
  ...
  const extractSyllabusFromPDF = async () => {
    ...
    setExtracting(true);
    try {
      const res = await apiClient.post(`/subjects/${subject._id}/syllabus/extract`, {
        fileData: subject.syllabusFile
      });
    ...
    <Shell>
      ...
      <div className="mb-8 flex space-x-1 rounded-xl bg-muted/50 p-1 w-fit">
        <button
          ...
          className={cx("rounded-lg px-6 py-2 text-sm font-semibold transition-all", activeTab === 'overview' ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:bg-background/50")}
        >
          Overview
        </button>
        <button
          onClick={() => setActiveTab('syllabus')}
          className={cx("rounded-lg px-6 py-2 text-sm font-semibold transition-all", activeTab === 'syllabus' ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:bg-background/50")}
        >
          Syllabus & Units
        </button>
      </div>
          ...
          <>
            <div className="lg:col-span-2 space-y-6">
                ...
                <div className="flex items-center gap-6">
                  <div className="relative flex h-24 w-24 items-center justify-center rounded-full bg-muted">
                    <svg className="absolute inset-0 h-full w-full -rotate-90 transform" viewBox="0 0 100 100">
                      ...
                      <circle cx="50" cy="50" r="45" fill="none" stroke="currentColor" strokeWidth="8" strokeDasharray={`${progressPercent * 2.83} 283`} className="text-accent transition-all duration-1000" strokeLinecap="round" />
                    </svg>
                    <span className="font-display text-xl">{progressPercent}%</span>
                  </div>
                  <div>
                    <p className="font-semibold text-lg">{completedTopics} of {totalTopics} Topics Completed</p>
                    <p className="text-sm text-muted-foreground mt-1">Keep up the momentum! You've got this.</p>
                    <Button variant="outline" className="mt-4 h-8" onClick={() => setActiveTab('syllabus')}>View Syllabus</Button>
                  </div>
                </div>
              </section>
              ...
              {subject.syllabusFile?.url && (
                <section className="rounded-2xl border border-card-border bg-card p-6">
                  <h3 className="font-display text-xl mb-3">Attached Syllabus</h3>
                  <div className="flex items-center justify-between p-4 rounded-xl border border-border bg-background">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-sm">{subject.syllabusFile.originalName}</p>
                        <p className="text-xs text-muted-foreground">PDF Document</p>
                      </div>
                    </div>
                    <a href={subject.syllabusFile.url} target="_blank" rel="noreferrer">
                      <Button variant="outline" className="h-8">View PDF</Button>
                    </a>
                  </div>
                </section>
              )}
            ...
            <div className="flex items-center justify-between bg-card border border-border rounded-2xl p-4">
              ...
              <div className="flex gap-3">
                {subject.syllabusFile?.url && units.length === 0 && (
                  <Button variant="outline" onClick={extractSyllabusFromPDF} disabled={extracting}>
                    {extracting ? 'Extracting...' : <><Wand2 size={16} className="mr-2"/> Auto-Extract from PDF</>}
                  </Button>
                )}
                <Button variant="default" onClick={() => setOpenUnitForm(true)}><Plus size={16} className="mr-2"/> Add Unit</Button>
              </div>
            </div>
            ...
            <div className="space-y-4">
              {units.length === 0 ? (
                <div className="rounded-xl border border-dashed border-border p-12 text-center text-muted-foreground">
                  ...
                  <p className="text-base font-semibold mb-2">No units defined</p>
                  <p className="text-sm max-w-md mx-auto">Break down your subject into manageable units and topics. You can manually add them or auto-extract from your syllabus PDF.</p>
                </div>
              ) : (
                    ...
                    return (
                      <div key={unit._id} className="rounded-xl border border-card-border bg-card overflow-hidden flex flex-col shadow-sm">
                        <div className="p-5 border-b border-border/50 bg-secondary/30">
                          <h3 className="font-display text-lg mb-1">Unit {idx + 1}: {unit.title}</h3>
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs font-semibold text-muted-foreground">{completedCount}/{unitTopics.length}</span>
                          </div>
                        </div>
                                ...
                                <div key={topic._id} className="flex items-start justify-between rounded-lg p-2 hover:bg-background group transition-colors">
                                  <button onClick={() => toggleTopic(topic)} className="flex items-start gap-3 text-left mt-0.5">
                                    {topic.status === 'completed' ? (
                                      ...
                                      <Circle size={16} className="text-muted-foreground opacity-50 group-hover:opacity-100 transition-opacity shrink-0 mt-0.5" />
                                    )}
                                    <span className={cx('text-sm transition-all leading-tight', topic.status === 'completed' && 'text-muted-foreground line-through opacity-70')}>{topic.name}</span>
                                  </button>
                                  <Link to={`/study-session?subject=${subject._id}&topic=${topic._id}`} className="rounded-md p-1.5 text-muted-foreground hover:bg-primary hover:text-primary-foreground transition-colors shrink-0 opacity-0 group-hover:opacity-100" title="Start Focus Session">
                                    <Play size={14} />
                                  </Link>
                                </div>
                              ))}
                        ...
                        <div className="p-3 border-t border-border/50 bg-background/50">
                          <button onClick={() => setOpenTopicFormForUnit(unit._id)} className="flex w-full items-center justify-center gap-2 rounded-lg py-2 text-xs font-semibold text-muted-foreground hover:bg-muted transition-colors">
