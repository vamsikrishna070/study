    ...
    <Shell>
      ...
      {query.isLoading ? (
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {data.map(s => (
            <article className="card-lift relative overflow-hidden rounded-2xl border border-card-border bg-card p-6" key={s.id} data-testid={`card-subject-${s.id}`}>
              ...
              <div className="flex items-start justify-between">
                ...
                <div className="flex gap-1">
                  <button onClick={() => { setEditing(s); setOpen(true); }} className="rounded-lg p-2 text-muted-foreground hover:bg-muted hover:text-foreground" data-testid={`button-edit-subject-${s.id}`}>
                    <Pencil size={15} />
                  </button>
                  <button onClick={() => remove(s)} className="rounded-lg p-2 text-muted-foreground hover:bg-destructive/10 hover:text-destructive" data-testid={`button-delete-subject-${s.id}`}>
                    <Trash2 size={15} />
                  </button>
                </div>
              </div>
