...
export default function SyllabusReviewModal({ subjectId, parsedData, onClose, onSuccess }) {
  ...
  const addUnit = () => {
    setUnits(prev => [...prev, { name: `Unit ${prev.length + 1}`, topics: [] }]);
  };

  const addTopic = (uIdx) => {
    const next = [...units];
    next[uIdx].topics.push({ name: 'New Topic', confidence: 'manual' });
    setUnits(next);
  };
  ...
  return (
    <Modal
      ...
      eyebrow={`Extracted ${units.length} units and ${totalTopics} topics`}
      onClose={onClose}
      footer={
    ...
    >
      <div className="space-y-6 max-h-[65vh] overflow-y-auto pr-2">
        {units.length === 0 ? (
          ...
          units.map((unit, uIdx) => (
            <div key={uIdx} className="rounded-2xl border border-border p-4 bg-card/60 shadow-sm">
              <div className="flex items-center gap-3 mb-3">
                <GripVertical size={16} className="text-muted-foreground/40 shrink-0" />
                <input
                  className={cx(inputClass, 'font-semibold text-base flex-1')}
                  value={unit.name}
              ...
              <div className="ml-7 space-y-2 border-l-2 border-border/60 pl-4">
                ...
                {unit.topics.length === 0 && (
                  <p className="text-xs text-muted-foreground py-1 italic">No topics in this unit yet.</p>
                )}
              </div>
            </div>
          ))
